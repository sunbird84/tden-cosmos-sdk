...
func DecodeArmor(armorStr string) (blockType string, headers map[string]string, data []byte, err error) {
	const maxArmorSize = 1 << 20
	if len(armorStr) == 0 {
		return "", nil, nil, io.EOF
	}
	if len(armorStr) > maxArmorSize {
		return "", nil, nil, armorCorrupt()
	}

	normalized := strings.ReplaceAll(armorStr, "\r\n", "\n")
	...
	if !strings.HasPrefix(begin, beginPrefix) || !strings.HasSuffix(begin, markerSuffix) {
		return "", nil, nil, armorCorrupt()
	}
	blockType = strings.TrimSuffix(strings.TrimPrefix(begin, beginPrefix), markerSuffix)
	if blockType == "" || strings.ContainsAny(blockType, "\r\n") {
		return "", nil, nil, armorCorrupt()
	}
	...
	for ; lineIndex < len(lines); lineIndex++ {
		...
		if len(line) > 256 || len(headers) >= 32 {
			return "", nil, nil, armorCorrupt()
		}
		key, value, ok := strings.Cut(line, ":")
		key = strings.TrimSpace(key)
		value = strings.TrimSpace(value)
		if !ok || !validArmorHeader(key, value) {
			return "", nil, nil, armorCorrupt()
		}
	...
	for ; lineIndex < len(lines); lineIndex++ {
		...
		case strings.HasPrefix(line, "="):
			if len(line) != 5 || expectedCRC != nil {
				return "", nil, nil, armorCorrupt()
			}
			var decodeErr error
			expectedCRC, decodeErr = base64.StdEncoding.DecodeString(line[1:])
			if decodeErr != nil || len(expectedCRC) != 3 {
				return "", nil, nil, armorCorrupt()
			}
			lineIndex++
			if lineIndex >= len(lines) || strings.TrimSpace(lines[lineIndex]) != endMarker {
				return "", nil, nil, armorCorrupt()
			}
			foundEnd = true
			lineIndex++
	...
	if expectedCRC != nil {
		crc := armorCRC24(0xb704ce, data) & 0xffffff
		actual := []byte{byte(crc >> 16), byte(crc >> 8), byte(crc)}
...
func validArmorHeader(key, value string) bool {
	if key == "" {
		return false
	}
...
func armorCRC24(crc uint32, data []byte) uint32 {
	const polynomial = 0x1864cfb
	for _, b := range data {
		crc ^= uint32(b) << 16
		for range 8 {
			crc <<= 1
...
func armorCorrupt() error {
	return fmt.Errorf("openpgp: invalid data: armor invalid")
}
